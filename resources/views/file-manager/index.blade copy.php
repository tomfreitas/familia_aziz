    ...
    <div class="w-100 d-block bg-light px-4 py-5 rounded-4 tela">
        <div class="row">
            <div class="col-md-6 col-12">
                <h4 class="mb-4 text-roxo">Gerenciador de arquivos</h4>
            </div>
        ...
        <div class="row">
            <div class="col-12 p-4 h-100 d-flex flex-column">
                <div class="gerenciador flex-1 overflow-scroll">
                    ...
                    <div class="row bg-roxo text-white py-1 px-3">
                        <div class="col-md-7 col-12 fw-bold">Nome</div>
                    ...
                    @foreach ($folder as $dir)
                        <form action="{{ route('file-manager.deldir', ['folder' => $dir['id']]) }}" method="POST" class="row lista-arquivos">
                            ...
                            <input type="hidden" name="caminho" id="caminho" value="{{ $directoryPath.$dir['pasta'] }}">
                            <div class="row py-1 px-4">
                                <div class="col-md-7 col-12">
                                    <a class="text-cinza text-decoration-none" href="{{ url('/file-manager/') }}@php echo '/'.$dir['pasta'] @endphp"><i class="fa-solid fa-folder text-aviso h5 my-0"></i> {{ $dir['pasta'] }}</a>
                                </div>
                                ...
                                <div class="col-md-2 col-12 d-flex align-items-end justify-content-end">
                                    <button type="button" class="fa-solid fa-pen h6 my-0 mx-3 text-roxo" data-bs-toggle="modal" data-bs-target="#edit{{$dir['id']}}"></button>
                                    <a href="{{ route('file-manager.zipfile', ['id' => $dir['id'], 'folder' => $dir['pasta']]) }}" class=""><i class="fa-solid fa-cloud-arrow-down h5 my-0 mx-3"></i></a>
                                    <button type="button" class="fa-solid fa-trash-can h5 my-0 mx-3 text-vermelho" data-bs-toggle="modal" data-bs-target="#Alert{{$dir['id']}}"></button>
                                </div>
                            </div>
                    ...
                    @foreach ($data as $file)
                        <form action="{{ route('file-manager.destroy', ['file' => $file['id']]) }}" method="POST" class="row lista-arquivos">
                            @csrf
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="caminho" id="caminho" value="{{ $directoryPath.$file['arquivo'] }}">
                            <div class="row py-1 px-4">
                                <div class="col-md-7 col-12">
                                    <a href="#!" class="text-black text-decoration-none info" data-bs-toggle="modal" data-bs-target="#Pic{{$file['id']}}"><i class="fa-solid fa-file text-roxo m-0 h5"></i> {{$file['arquivo']}}</a>
                                </div>
                                ...
                                <div class="col-md-2 col-12 d-flex align-items-center justify-content-end">
                                    <button type="button" class="fa-solid fa-pen h6 my-0 mx-3 text-roxo" data-bs-toggle="modal" data-bs-target="#editFile{{$file['id']}}"></button>
                                    <a href="{{ url('/')}}{{$directory.$file['arquivo'] }}" download="{{$file['arquivo']}}" class=""><i class="fa-solid fa-cloud-arrow-down h5 my-0 mx-3"></i></a>
                                    <button type="button" class="fa-solid fa-trash-can h5 my-0 mx-3 text-vermelho" data-bs-toggle="modal" data-bs-target="#Alert{{$file['id']}}"></button>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body p-0 bg-roxo">
                                                @if ($file['tipo'] == 'image/png' || $file['tipo'] == 'image/jpeg')
                                                    <img src="{{ url('/')}}{{$directory.$file['arquivo'] }}" class="img-fluid" alt="">
                                                @elseif ($file['tipo'] == 'audio/mpeg')
                                                    <audio controls class="w-100 d-block" id="myAudio{{$file['id']}}">
                                                        <source src="{{ url('/')}}{{$directory.$file['arquivo'] }}" type="audio/mp3">
                                                ...
                                                @elseif ($file['tipo'] == 'video/x-matroska' || $file['tipo'] == 'video/mp4' || $file['tipo'] == 'video/mpeg' || $file['tipo'] == 'video/quicktime')
                                                    <video width="100%" height="450"  id="myVideo{{$file['id']}}" controls>
                                                        <source src="{{ url('/')}}{{$directory.$file['arquivo'] }}">
                                            ...
                                            <div class="modal-footer justify-content-center">
                                                <a href="{{ url('/')}}{{$directory.$file['arquivo'] }}" download="{{$file['arquivo']}}" class="btn btn-sm btn-verde px-3 rounded-pill mx-2"><i class="fa-solid fa-cloud-arrow-down h5 my-0 mx-3"></i> Fazer download</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
