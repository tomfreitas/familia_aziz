            ...
            <div class="col-12 p-4 h-100 d-flex flex-column">
                <div class="gerenciador flex-1 overflow-scroll">
                    ...
                    <div class="row bg-cinza text-white py-1 px-3">
                        ...
                        <div class="col-md-1 col-12 fw-bold">Tipo</div>
                    ...
                    @foreach ($folder as $dir)
                        <form action="{{ route('file-manager.deldir', ['folder' => $dir['pasta']]) }}" method="POST" class="row lista-arquivos">
                            @csrf
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="caminho" id="caminho" value="{{ $directoryPath.'/'.$dir['pasta'] }}">
                            <div class="row py-1 px-4">
                                <div class="col-md-7 col-12">
                                    <a class="text-cinza text-decoration-none d-flex align-items-center" href="{{ url()->current().'/'.$dir['pasta'] }}">
                                        <span class="material-symbols-outlined symbol-filled text-aviso">folder</span>&nbsp;{{ $dir['pasta'] }}
                                    </a>
                                </div>
                                <div class="col-md-2 col-12">Pasta</div>
                                ...
                                <div class="col-md-2 col-12 d-flex align-items-center justify-content-end">
                                    ...
                                    <a href="{{ route('file-manager.zipfile', ['id' => $dir['id'], 'folder' => $dir['pasta']]) }}" class="d-inline-flex align-items-center justify-content-center mx-1">
                        ...
                        <div class="modal fade" id="edit{{$dir['id']}}" tabindex="-1" role="dialog" aria-labelledby="edit{{$dir['id']}}ModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="edit{{$dir['pasta']}}ModalLabel">Renomear Pasta</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form method="post" action="{{ route('file-manager.rename', ['folder' => $dir['pasta']]) }}">
                                            @csrf
                                            ...
                                            <button type="submit" class="btn w-100 btn-sm px-5 btn-verde rounded-pill">Renomear</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                    ...
                    @foreach ($data as $file)
                        <form action="{{ route('file-manager.destroy', ['file' => $file['id']]) }}" method="POST" class="row lista-arquivos">
                            ...
                            <div class="row py-1 px-4">
                                ...
                                <div class="col-md-1 col-12">Arquivo</div>
                                ...
                                <div class="col-md-2 col-12 d-flex align-items-center justify-content-end">
                                    ...
                                    <a href="{{ url('/')}}{{$showdir.$file['arquivo'] }}" download="{{$file['arquivo']}}" class="d-inline-flex align-items-center justify-content-center mx-1">
                                        <span class="material-symbols-outlined text-azul me-1">download</span>
                                    </a>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body p-0 bg-roxo">
                                                @if ($file['tipo'] == 'image/png' || $file['tipo'] == 'image/jpeg')
                                                    <img src="{{ url('/')}}{{$showdir.$file['arquivo'] }}" class="img-fluid" alt="">
                                                @elseif ($file['tipo'] == 'audio/mpeg')
                                                    <audio controls class="w-100 d-block" id="myAudio">
                                                        <source src="{{ url('/')}}{{$showdir.$file['arquivo'] }}" type="audio/mp3">
                                                        Seu navegador não suporta o elemento de áudio.
                                                    </audio>
                                                @elseif ($file['tipo'] == 'video/x-matroska' || $file['tipo'] == 'video/mp4' || $file['tipo'] == 'video/mpeg' || $file['tipo'] == 'video/quicktime')
                                                    <video width="100%" height="450" id="myVideo" controls>
                                                        <source src="{{ url('/')}}{{$showdir.$file['arquivo'] }}">
                                            ...
                                            <div class="modal-footer justify-content-center">
                                                <a href="{{ url('/')}}{{$showdir.$file['arquivo'] }}" download="{{$file['arquivo']}}" class="btn btn-sm btn-verde px-3 rounded-pill mx-2"><i class="fa-solid fa-cloud-arrow-down h5 my-0 mx-3"></i> Fazer download</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
